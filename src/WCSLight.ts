...
export class WCSLight {
   ...
   */
  static async fitsCutoutToHiPS(
    center: Point,
    radius: number,
    pxsize: number,
    filePath: string,
  ): Promise<FITSList | null> {
    ...
    const outTilesRaDecList: TilesRaDecList2 | null =
      HiPSProjection.getImageRADecList(center, radius, pxsize, HiPS_TILE_WIDTH);
    ...
    const fitsFileList = HiPSProjection.getFITSFiles(
      outTilesRaDecList,
      inProjection.getFITSHeader(),
      pxsize,
      HiPS_TILE_WIDTH,
    );
  ...
  static async extractProjectionType(
    filePath: string,
  ): Promise<AbstractProjection | null> {
    const fitsFile = await FITSParser.loadFITSFile(filePath);
    ...
    const ctype = String(primary.header.findById("CTYPE1")?.value);
  ...
  static async hipsCutoutToFITS(
    ...
    if (!hipsOrder) {
      const healpix = HiPSHelper.getHelpixBypxAngSize(
        pixelAngSize,
        TILE_WIDTH,
        hipsMaxOrder,
      );
    ...
    const outRADecList: TilesRaDecList2 = outproj.getImageRADecList(
      center,
      radius,
      pixelAngSize,
      naxisWidth,
    ...
    const raDecWithValues = await HiPSProjection.world2pix(
      outRADecList,
      hipsOrder,
      isGalactic,
      TILE_WIDTH,
