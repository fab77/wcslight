...
export class MercatorProjection extends AbstractProjection {
  ...
  async initFromFile(infile: string): Promise<PrimaryHDU> {
    ...
    if (fits.naxis !== 2) {
      throw new Error(
        `MercatorProjection requires a 2D FITS image, got NAXIS=${fits.naxis}`,
      );
    }
    ...
    const centerY = mercatorYdegFromLatDeg(this.cdecDeg);

    this.mindec = centerY - (this.pxsize * this.naxis2) / 2;
  ...
  prepareHeader(
    ...
    fitsheader.insert(
      new FITSHeaderItem("DATAMIN", BZERO + BSCALE * minValue, ""),
    );
    fitsheader.insert(
      new FITSHeaderItem("DATAMAX", BZERO + BSCALE * maxValue, ""),
    );
    fitsheader.insert(
