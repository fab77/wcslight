...
exports.HiPSIntermediateProj = void 0;
var healpixjs_1 = require("healpixjs");
var CoordsType_js_1 = require("../../model/CoordsType.js");
var NumberType_js_1 = require("../../model/NumberType.js");
var Point_js_1 = require("../../model/Point.js");
var Utils_js_1 = require("../../model/Utils.js");
var HiPSIntermediateProj = /** @class */ (function () {
    function HiPSIntermediateProj() {
    }
    HiPSIntermediateProj.setupByTile = function (tileno, hp) {
        var xyGridProj = {
            "min_y": NaN,
            "max_y": NaN,
            "min_x": NaN,
        ...
        var cornersVec3 = hp.getBoundariesWithStep(tileno, 1);
        var pointings = [];
        for (var i = 0; i < cornersVec3.length; i++) {
            pointings[i] = new healpixjs_1.Pointing(cornersVec3[i]);
            if (i >= 1) {
                var a = pointings[i - 1].phi;
                var b = pointings[i].phi;
        ...
        for (var j = 0; j < pointings.length; j++) {
            ...
            var raRad = pointings[j].phi;
            // projection on healpix grid
            var p = new Point_js_1.Point(CoordsType_js_1.CoordsType.ASTRO, NumberType_js_1.NumberType.RADIANS, raRad, decRad);
            var xyDeg = HiPSIntermediateProj.world2intermediate(p.getAstro());
    ...
    HiPSIntermediateProj.world2intermediate = function (ac) {
        var x_grid = NaN;
        var y_grid = NaN;
        if (Math.abs(ac.decRad) <= HiPSIntermediateProj.THETAX) { // equatorial belts
            x_grid = ac.raDeg;
            y_grid = healpixjs_1.Hploc.sin(ac.decRad) * HiPSIntermediateProj.K * 90 / HiPSIntermediateProj.H;
        }
            ...
            var sigma = Math.sqrt(HiPSIntermediateProj.K * (1 - Math.abs(healpixjs_1.Hploc.sin(ac.decRad))));
            var phi_c = -180 + (2 * Math.floor(((ac.raDeg + 180) * HiPSIntermediateProj.H / 360) + ((1 - w) / 2)) + w) * (180 / HiPSIntermediateProj.H);
    ...
    HiPSIntermediateProj.intermediate2pix = function (x, y, xyGridProj, pxXtile) {
        var xInterval = Math.abs(xyGridProj.max_x - xyGridProj.min_x);
        var yInterval = Math.abs(xyGridProj.max_y - xyGridProj.min_y);
        var i_norm;
        var j_norm;
    ...
    HiPSIntermediateProj.pix2intermediate = function (i, j, xyGridProj, naxis1, naxis2) {
        /**
        ...
        var i_norm = (i + 0.5) / cnaxis1;
        var j_norm = (j + 0.5) / cnaxis2;
        var xInterval = Math.abs(xyGridProj.max_x - xyGridProj.min_x) / 2.0;
        var yInterval = Math.abs(xyGridProj.max_y - xyGridProj.min_y) / 2.0;
        var yMean = (xyGridProj.max_y + xyGridProj.min_y) / 2.0;
        // bi-linear interpolation
        var x = xyGridProj.max_x - xInterval * (i_norm + j_norm);
        var y = yMean - yInterval * (j_norm - i_norm);
        return [x, y];
    };
    HiPSIntermediateProj.intermediate2world = function (x, y) {
        var phiDeg = NaN;
        var thetaDeg = NaN;
        var Yx = 90 * (HiPSIntermediateProj.K - 1) / HiPSIntermediateProj.H;
        if (Math.abs(y) <= Yx) { // equatorial belts
            phiDeg = x;
            thetaDeg = (0, Utils_js_1.radToDeg)(Math.asin((y * HiPSIntermediateProj.H) / (90 * HiPSIntermediateProj.K)));
        }
        else if (Math.abs(y) > Yx) { // polar regions
            var sigma = (HiPSIntermediateProj.K + 1) / 2 - Math.abs(y * HiPSIntermediateProj.H) / 180;
            var thetaRad = healpixjs_1.Hploc.asin(1 - (sigma * sigma) / HiPSIntermediateProj.K);
            var w = 0; // omega
            if (HiPSIntermediateProj.K % 2 !== 0 || thetaRad > 0) { // K odd or thetax > 0
                w = 1;
            }
            var x_c = -180 + (2 * Math.floor((x + 180) * HiPSIntermediateProj.H / 360 + (1 - w) / 2) + w) * (180 / HiPSIntermediateProj.H);
            phiDeg = x_c + (x - x_c) / sigma;
            thetaDeg = (0, Utils_js_1.radToDeg)(thetaRad);
            if (y <= 0) {
                thetaDeg *= -1;
            }
        }
        ...
        var p = new Point_js_1.Point(CoordsType_js_1.CoordsType.ASTRO, NumberType_js_1.NumberType.DEGREES, phiDeg, thetaDeg);
        return p;
    };
    ...
    HiPSIntermediateProj.THETAX = healpixjs_1.Hploc.asin((HiPSIntermediateProj.K - 1) / HiPSIntermediateProj.K);
    return HiPSIntermediateProj;
}());
exports.HiPSIntermediateProj = HiPSIntermediateProj;
